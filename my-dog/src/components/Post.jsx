...
function calculateTimeDifference(postDate) {
  ...
  function Post({image, date, likes, author, description, comments}) {
    const [timeDiff, setTimeDiff] = useState(calculateTimeDifference(date));
  
    // Calculate and update the time difference every minute
    useEffect(() => {
      const intervalId = setInterval(() => {
        setTimeDiff(calculateTimeDifference(date));
